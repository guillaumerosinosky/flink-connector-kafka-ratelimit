 ...
 */
@Internal
public class KafkaRatelimitDynamicTableFactory extends KafkaDynamicTableFactory {

  private static final Logger LOG = LoggerFactory.getLogger(KafkaRatelimitDynamicTableFactory.class);
  ...
  private static final ConfigOption<Double> RATE_LIMIT = ConfigOptions.key("rate.limit")
      .doubleType()
      .noDefaultValue()
      .withDescription("Rate limit source.");

  public static final String IDENTIFIER = "kafka-ratelimit";
  ...
  public DynamicTableSource createDynamicTableSource(Context context) {
    ...
    final Optional<DecodingFormat<DeserializationSchema<RowData>>> keyDecodingFormat = getKeyDecodingFormat(
        helper);

    final DecodingFormat<DeserializationSchema<RowData>> valueDecodingFormat = getValueDecodingFormat(
        helper);
    ...
    final Optional<Long> partitionDiscoveryInterval = tableOptions
        .getOptional(SCAN_TOPIC_PARTITION_DISCOVERY).map(Duration::toMillis);
    properties.setProperty(
        KafkaSourceOptions.PARTITION_DISCOVERY_INTERVAL_MS.key(),
        partitionDiscoveryInterval.orElse(-1L).toString());
    properties.setProperty(RATE_LIMIT.key(), helper.getOptions().get(RATE_LIMIT).toString());
  ...
  public DynamicTableSink createDynamicTableSink(Context context) {
    ...
    final Optional<EncodingFormat<SerializationSchema<RowData>>> keyEncodingFormat = getKeyEncodingFormat(
        helper);
  ...
  protected KafkaRatelimitDynamicSource createKafkaTableSource(
    ...
    LOG.info("createKafkaTableSource...");
    return new KafkaRatelimitDynamicSource(
        physicalDataType,
        keyDecodingFormat,
        valueDecodingFormat,
        keyProjection,
