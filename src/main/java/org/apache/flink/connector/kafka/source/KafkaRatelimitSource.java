 ...
 *
 * <pre>{@code
 * KafkaRatelimitSource<String> source = KafkaSource
 *         .<String>builder()
 ...
 */
public class KafkaRatelimitSource<OUT> extends KafkaSource<OUT> {
    private static final Logger LOG = LoggerFactory.getLogger(KafkaRatelimitSource.class);
    public static final String RATE_LIMITER_CONFIG = "rate.limit";
    private static final long serialVersionUID = -8755372893283732099L; // +1
    // Users can choose only one of the following ways to specify the topics to
    // consume from.
     ...
     */
    public static <OUT> KafkaRatelimitSourceBuilder<OUT> builderRatelimit() {
        return new KafkaRatelimitSourceBuilder<>();
    }
    ...
    SourceReader<OUT, KafkaPartitionSplit> createReader(
        ...
        double rateLimit;
        try {
            rateLimit = DecimalFormat.getNumberInstance()
                    .parse(props.getProperty(KafkaRatelimitSource.RATE_LIMITER_CONFIG)).doubleValue();
        } catch (ParseException e) {
            Log.error("error while parsing rateLimit parameter, setting to 1000", e);
            rateLimit = 10;
        } catch (NullPointerException e) {
            Log.error("error while parsing rateLimit parameter, setting to 1000", e);
            rateLimit = 10;
        }
        LOG.info(String.format("create KafkaRatelimitRecordEmitter with %f", rateLimit));
        KafkaRatelimitRecordEmitter<OUT> recordEmitter = new KafkaRatelimitRecordEmitter<OUT>(rateLimit);
