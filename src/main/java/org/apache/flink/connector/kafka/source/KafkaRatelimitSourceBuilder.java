...
/**
 * The @builder class for {@link KafkaRatelimitSource} to make it easier for the
 * users to construct a {@link
 * KafkaRatelimitSource}.
 *
 * <p>
 ...
 *
 * <p>
 * By default the KafkaSource runs in an
 * {@link Boundedness#CONTINUOUS_UNBOUNDED} mode and never
 * stops until the Flink job is canceled or fails. To let the KafkaSource run in
 * {@link
 ...
 * KafkaSource.
 */
public class KafkaRatelimitSourceBuilder<OUT> {
     ...
     */
    public KafkaRatelimitSourceBuilder<OUT> setBootstrapServers(String bootstrapServers) {
     ...
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setGroupId(String groupId) {
        return setProperty(ConsumerConfig.GROUP_ID_CONFIG, groupId);
    }

    /**
     * Set a list of topics the KafkaSource should consume from. All the topics in
     * the list should
     * have existed in the Kafka cluster. Otherwise an exception will be thrown. To
     * allow some of
     * the topics to be created lazily, please use {@link #setTopicPattern(Pattern)}
     * instead.
     *
     ...
     */
    public KafkaRatelimitSourceBuilder<OUT> setTopics(List<String> topics) {
     ...
     * @see org.apache.kafka.clients.consumer.KafkaConsumer#subscribe(Collection)
     */
    public KafkaRatelimitSourceBuilder<OUT> setTopics(String... topics) {
     ...
     */
    public KafkaRatelimitSourceBuilder<OUT> setTopicPattern(Pattern topicPattern) {
     ...
     */
    public KafkaRatelimitSourceBuilder<OUT> setPartitions(Set<TopicPartition> partitions) {
     ...
     *
     * <p>
     * The following {@link OffsetsInitializer}s are commonly used and provided out
     * of the box.
     * Users can also implement their own {@link OffsetsInitializer} for custom
     * behaviors.
     *
     * <ul>
     * <li>{@link OffsetsInitializer#earliest()} - starting from the earliest
     * offsets. This is
     * also the default {@link OffsetsInitializer} of the KafkaSource for starting
     * offsets.
     ...
     * - starting from the committed offsets of the consumer group. If there is no
     * committed
     ...
     * <li>{@link OffsetsInitializer#offsets(Map)} - starting from the specified
     * offsets for each
     * partition.
     * <li>{@link OffsetsInitializer#timestamp(long)} - starting from the specified
     * timestamp for
     * each partition. Note that the guarantee here is that all the records in Kafka
     * whose
     * {@link org.apache.kafka.clients.consumer.ConsumerRecord#timestamp()} is
     * greater than
     * the given starting timestamp will be consumed. However, it is possible that
     * some
     * consumer records whose timestamp is smaller than the given starting timestamp
     * are also
     * consumed.
     * </ul>
     *
     * @param startingOffsetsInitializer the {@link OffsetsInitializer} setting the
     *                                   starting offsets
     *                                   for the Source.
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setStartingOffsets(
     ...
     * {@link Boundedness#CONTINUOUS_UNBOUNDED} manner
     * and thus never stops until the Flink job fails or is canceled. To let the
     * KafkaSource run as
     * a streaming source but still stops at some point, one can set an
     * {@link OffsetsInitializer}
     * to specify the stopping offsets for each partition. When all the partitions
     * have reached
     * their stopping offsets, the KafkaSource will then exit.
     *
     * <p>
     * This method is different from {@link #setBounded(OffsetsInitializer)} that
     * after setting
     * the stopping offsets with this method, {@link KafkaSource#getBoundedness()}
     * will still return
     * {@link Boundedness#CONTINUOUS_UNBOUNDED} even though it will stop at the
     * stopping offsets
     * specified by the stopping offsets {@link OffsetsInitializer}.
     *
     * <p>
     * The following {@link OffsetsInitializer} are commonly used and provided out
     * of the box.
     * Users can also implement their own {@link OffsetsInitializer} for custom
     * behaviors.
     *
     * <ul>
     * <li>{@link OffsetsInitializer#latest()} - stop at the latest offsets of the
     * partitions when
     * the KafkaSource starts to run.
     * <li>{@link OffsetsInitializer#committedOffsets()} - stops at the committed
     * offsets of the
     * consumer group.
     * <li>{@link OffsetsInitializer#offsets(Map)} - stops at the specified offsets
     * for each
     * partition.
     * <li>{@link OffsetsInitializer#timestamp(long)} - stops at the specified
     * timestamp for each
     * partition. The guarantee of setting the stopping timestamp is that no Kafka
     * records
     * whose {@link org.apache.kafka.clients.consumer.ConsumerRecord#timestamp()} is
     * greater
     * than the given stopping timestamp will be consumed. However, it is possible
     * that some
     * records whose timestamp is smaller than the specified stopping timestamp are
     * not
     * consumed.
     * </ul>
     *
     * @param stoppingOffsetsInitializer The {@link OffsetsInitializer} to specify
     *                                   the stopping
     *                                   offset.
     * @return this KafkaSourceBuilder.
     * @see #setBounded(OffsetsInitializer)
     */
    public KafkaRatelimitSourceBuilder<OUT> setUnbounded(OffsetsInitializer stoppingOffsetsInitializer) {
        this.boundedness = Boundedness.CONTINUOUS_UNBOUNDED;
     ...
     * {@link
     * OffsetsInitializer} to specify the stopping offsets for each partition. When
     * all the
     * partitions have reached their stopping offsets, the KafkaSource will then
     * exit.
     *
     * <p>
     * This method is different from {@link #setUnbounded(OffsetsInitializer)} that
     * after setting
     * the stopping offsets with this method, {@link KafkaSource#getBoundedness()}
     * will return
     * {@link Boundedness#BOUNDED} instead of
     ...
     */
    public KafkaRatelimitSourceBuilder<OUT> setBounded(OffsetsInitializer stoppingOffsetsInitializer) {
     ...
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setDeserializer(
     ...
     *
     * @param deserializationSchema the {@link DeserializationSchema} to use for
     *                              deserialization.
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setValueOnlyDeserializer(
     ...
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setClientIdPrefix(String prefix) {
        return setProperty(KafkaSourceOptions.CLIENT_ID_PREFIX.key(), prefix);
    }

    /**
     * Set an arbitrary property for the KafkaSource and KafkaConsumer. The valid
     * keys can be found
     * in {@link ConsumerConfig} and {@link KafkaSourceOptions}.
     *
     * <p>
     * Note that the following keys will be overridden by the builder when the
     * KafkaSource is
     * created.
     *
     * <ul>
     * <li><code>key.deserializer</code> is always set to
     * {@link ByteArrayDeserializer}.
     * <li><code>value.deserializer</code> is always set to
     * {@link ByteArrayDeserializer}.
     * <li><code>auto.offset.reset.strategy</code> is overridden by {@link
     * OffsetsInitializer#getAutoOffsetResetStrategy()} for the starting offsets,
     * which is by
     * default {@link OffsetsInitializer#earliest()}.
     * <li><code>partition.discovery.interval.ms</code> is overridden to -1 when
     * {@link
     ...
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setProperty(String key, String value) {
        props.setProperty(key, value);
        return this;
    }

    /**
     * Set arbitrary properties for the KafkaSource and KafkaConsumer. The valid
     * keys can be found
     * in {@link ConsumerConfig} and {@link KafkaSourceOptions}.
     *
     * <p>
     ...
     * @return this KafkaSourceBuilder.
     */
    public KafkaRatelimitSourceBuilder<OUT> setProperties(Properties props) {
     ...
     */
    public KafkaRatelimitSource<OUT> build() {
        LOG.info("build");
        sanityCheck();
        parseAndSetRequiredProperties();
        return new KafkaRatelimitSource<>(
                subscriber,
    ...
    private void parseAndSetRequiredProperties() {
        ...
        // If the client id prefix is not set, reuse the consumer group id as the client
        // id prefix,
        ...
        maybeOverride(
                ...
                props.containsKey(ConsumerConfig.GROUP_ID_CONFIG)
                        ...
                        : "KafkaSourceRateLimit-" + new Random().nextLong(),
                false);
    }
    ...
    private boolean offsetCommitEnabledManually() {
        boolean autoCommit = props.containsKey(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG)
                && Boolean.parseBoolean(
                        props.getProperty(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG));
        boolean commitOnCheckpoint = props.containsKey(KafkaSourceOptions.COMMIT_OFFSETS_ON_CHECKPOINT.key())
                && Boolean.parseBoolean(
                        props.getProperty(
